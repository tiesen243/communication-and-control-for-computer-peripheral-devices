...
void esp8266_restart(void) {
  ...
  _esp8266_waitFor((unsigned char *)"OK");       // Wait for "OK"
...
void esp8266_echoCmds(bool echo) {
  _esp8266_print((unsigned char *)"ATE"); // Command
...
void esp8266_mode(unsigned char mode) {
  _esp8266_print((unsigned char *)"AT+CWMODE="); // Command
...
void esp8266_trans_mode(unsigned char mode) {
  _esp8266_print((unsigned char *)"AT+CIPMODE="); // Command
  _esp8266_putch(mode + '0');
  _esp8266_print((unsigned char *)"\r\n");
  _esp8266_waitFor((unsigned char *)"OK"); // Wait for "OK"
}
...
void esp8266_connect(unsigned char *ssid, unsigned char *pass) {
  _esp8266_print((unsigned char *)"AT+CWJAP=\""); // Command
  _esp8266_print(ssid);
  _esp8266_print((unsigned char *)"\",\"");
  _esp8266_print(pass);
  _esp8266_print((unsigned char *)"\"\r\n");
  _esp8266_waitFor((unsigned char *)"OK");
}
...
void esp8266_receive(unsigned char *store_in) {
  ...
  _esp8266_waitFor((unsigned char *)"+IPD,"); // Wait for: +IPD - Read data
  do {
...
void esp8266_close(void) {
  _esp8266_print((unsigned char *)"AT+CIPCLOSE\r\n");
  _esp8266_waitFor((unsigned char *)"OK");
}
